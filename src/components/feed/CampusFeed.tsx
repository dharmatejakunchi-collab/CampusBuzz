...
export const CampusFeed: React.FC<CampusFeedProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-900 to-amber-950/80 p-6 sm:p-8 text-white shadow-2xl border border-slate-800/80">
        <div className="relative z-10 max-w-3xl space-y-3.5">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-orange-500/20 text-orange-400 border border-orange-500/30 text-xs font-black tracking-wider uppercase backdrop-blur-md">
          ...
          <p className="text-sm sm:text-base text-slate-300 leading-relaxed font-normal">
            Every post is driven by an intent hashtag: join live order rooms with <span className="text-amber-400 font-bold">#foodsplit</span>, ride shares with <span className="text-emerald-400 font-bold">#cabsplit</span>, deals on <span className="text-violet-400 font-bold">#resell</span>, or direct claims with <span className="text-rose-400 font-bold">#lost</span> & <span className="text-cyan-400 font-bold">#found</span>.
          </p>

          {/* Quick Metrics ticker */}
          <div className="pt-2 flex flex-wrap items-center gap-2.5 sm:gap-3 text-xs font-bold">
            <div className="bg-white/10 dark:bg-black/30 backdrop-blur-md px-3.5 py-1.5 rounded-xl flex items-center space-x-2 border border-white/15 text-amber-300">
              <UtensilsCrossed className="w-4 h-4 text-amber-400" />
              <span>{activeSplitsCount} Active Splits</span>
            </div>
            <div className="bg-white/10 dark:bg-black/30 backdrop-blur-md px-3.5 py-1.5 rounded-xl flex items-center space-x-2 border border-white/15 text-purple-300">
              <Tag className="w-4 h-4 text-purple-400" />
              <span>{activeResellsCount} Resale Items</span>
            </div>
            <div className="bg-white/10 dark:bg-black/30 backdrop-blur-md px-3.5 py-1.5 rounded-xl flex items-center space-x-2 border border-white/15 text-rose-300">
        ...
        <div className="absolute right-0 top-0 bottom-0 w-1/2 bg-gradient-to-l from-orange-500/20 via-amber-500/10 to-transparent pointer-events-none" />
      </div>

      {/* Urgent Countdown Alert Strip (if any posts expiring within 30m) */}
      {urgentSplits.length > 0 && (
        <div className="p-3.5 rounded-2xl bg-gradient-to-r from-rose-500/15 via-orange-500/10 to-amber-500/15 border border-rose-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs animate-in fade-in">
          <div className="flex items-center space-x-2.5 font-bold text-rose-700 dark:text-rose-300">
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl shadow-xs shrink-0 self-end sm:self-center transition-all"
          >
      ...
      <div className="space-y-3">
              ...
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-xs font-extrabold shrink-0 flex items-center space-x-1.5 transition-all duration-200 ${
                  isSelected
                    ...
                    : `bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-800 hover:border-orange-300 dark:hover:border-slate-700`
                }`}
              >
                ...
                <span className={`text-[10px] px-1.5 py-0.2 rounded-md font-mono ${isSelected ? 'bg-white/30 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
                  {count}
                </span>
              </button>
            );
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-xs">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-12 py-2 text-xs rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500 font-medium"
            />
            {searchQuery && (
              <button
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center space-x-1.5">
              <Building className="w-3.5 h-3.5 text-slate-400 hidden sm:inline" />
              <select
                value={selectedHostel}
                onChange={(e) => setSelectedHostel(e.target.value)}
                className="px-3 py-2 text-xs font-semibold rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/80 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-orange-500"
              >
            ...
            <button
              ...
              className="sm:hidden px-3.5 py-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl text-xs font-bold flex items-center space-x-1"
            >
