...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <>
      <header className="sticky top-0 z-40 bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl border-b border-slate-200/80 dark:border-slate-800 transition-colors shadow-xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            >
              <div className={`w-10 h-10 rounded-2xl bg-gradient-to-tr ${currentTheme.gradient} flex items-center justify-center shadow-lg text-white font-black text-xl tracking-wider group-hover:scale-105 transition-transform duration-200`}>
            ...
            <nav className="hidden md:flex items-center space-x-1 bg-slate-100/90 dark:bg-slate-900/90 p-1.5 rounded-2xl border border-slate-200/80 dark:border-slate-800 relative">
              <button
                id="nav-feed-tab"
                onClick={() => handleTabSelect('feed')}
              ...
              >
                {activeTab === 'feed' && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-white dark:bg-slate-800 rounded-xl shadow-xs -z-10"
                ...
                <button
                  ...
                  className={`relative z-10 flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-bold transition-colors duration-150 ${
                    activeTab === 'admin'
                      ...
                      : 'text-purple-900/80 dark:text-purple-300/80 hover:text-purple-950 dark:hover:text-white'
                  }`}
                >
                  {activeTab === 'admin' && (
                    <motion.div
                      layoutId="header-active-tab-pill"
                      className="absolute inset-0 bg-purple-100 dark:bg-purple-950/70 rounded-xl shadow-xs border border-purple-300/60 -z-10"
                      transition={{ type: 'spring', stiffness: 380, damping: 30 }}
                    />
                  )}
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className="flex items-center space-x-1.5 p-2 sm:px-3 sm:py-2 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all text-xs font-bold"
              >
                <div className="flex items-center -space-x-1">
                  {currentTheme.previewColors.map((c, i) => (
                    <span
                      key={i}
                      className="w-2.5 h-2.5 rounded-full border border-white/60 dark:border-slate-900"
              ...
              <button
                ...
                className={`flex items-center space-x-1.5 px-3.5 py-2 bg-gradient-to-r ${currentTheme.buttonGradient} text-white font-bold text-xs rounded-xl shadow-md active:scale-95 transition-all duration-150`}
              >
              ...
              <div className="relative">
                <button
                  ...
                  className="flex items-center space-x-2 p-1.5 sm:px-2.5 sm:py-1.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all text-left"
                >
                  <img
                    src={profile?.photoURL || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150'}
                    alt={profile?.displayName || 'User'}
                    className="w-7 h-7 rounded-xl object-cover border border-slate-300 dark:border-slate-700 shadow-xs"
                  />
                  <div className="hidden sm:block text-xs">
                    <div className="font-bold text-slate-900 dark:text-slate-100 leading-tight max-w-[95px] truncate">
